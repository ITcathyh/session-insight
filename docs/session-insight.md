...
在浏览器打开服务后，点击右上角「同步会话」：

1. 点击「选择文件」或「选择目录」，从当前电脑选择会话日志。
2. 在列表中勾选要同步的文件，也可全选或取消选择。列表显示数量、总大小和超限文件；选择后不会立即上传。
3. 点击「同步所选 N 个文件」才会上传到当前服务，显示进度、新增、更新、跳过数量和警告。

同步完成后更新页面数据；单文件成功时可直接打开分析。请求失败时保留未完成的所选文件，由用户点击重试；已完成批次不会再次提交。取消选择不会上传。后续日志变化需要用户重新选择并发起同步，不在后台定时执行。

远端服务无需访问本机目录：浏览器只提交用户选中的文件，服务端沿用现有导入接口解析。选择 Claude 子 agent 时建议选择包含 `subagents/` 的父目录，以保留目录关系。

### 手动命令行同步（可选）

也可以在日志所在电脑的仓库根目录手动运行一次：
...
单文件上限为 **32 MiB**，单轮最多处理 **512 MiB / 10,000 个待处理文件**。未变化文件不占上传预算，剩余文件可在下次手动执行时继续处理。超过单文件上限的文件不会拆分或截断，需要在日志所在机器运行服务并使用页面扫描分析。跳过、读取期间变化、导入失败及解析警告均在终端报告；失败文件在下次手动执行时重试。运行有跳过或失败时返回非零状态，执行一次后退出。命令行同步完成后刷新浏览器查看最新分析。

页面里的「扫描服务端」读取远端当前用户的会话目录；「选择文件 / 目录」从浏览器所在电脑选择，由「同步所选」发起上传。页面不直接读取另一台机器的目录。

## 导入和分析

选择 JSONL 文件或目录后，在列表中勾选文件并点击「同步所选」；单文件成功后直接打开该 session；有跳过项或解析警告时先保留反馈供检查。目录中的所选文件按每批最多 20 文件、32 MB 分批提交，显示进度和累计结果；中途失败时说明已完成数量，已导入部分保留，剩余所选文件可手动重试。Codex、Claude Code 与 TraeX 文件会按内容自动识别；TraeX 的 `model_provider` 为 `trae` 或 `traex` 时都会归一为 provider `traex`。导入结束后可以按 provider、model、项目、时间、工具、Skill、错误、纠偏、上下文风险和关键词搜索。
