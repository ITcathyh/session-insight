...
export interface RunFilters { q: string; provider: string; tool: string; skill: string; model?: string; from: string; to: string; error?: string; correction?: string; contextRisk?: string }
export interface NameCount { name: string; count: number }
export interface ProjectStat { name: string; runs: number; tokens: number; durationMs: number; failures: number }
export interface ToolStat { name: string; calls: number; failures: number }
export interface DayStat { date: string; runs: number; tokens: number; failures: number }
/** Aggregate over every run matching the active filters, not just the loaded page. */
export interface Stats { runCount: number; tokens: TokenBuckets; tokenRunCount: number; toolCalls: number; toolFailures: number; failedRunCount: number; contextRiskRuns: number; correctionRuns: number; subagentRuns: number; wallDurationMs: number; activeDurationMs: number; idleDurationMs: number; cacheHitRatio?: number; providers: NameCount[]; models: NameCount[]; projects: ProjectStat[]; tools: ToolStat[]; daily: DayStat[] }
