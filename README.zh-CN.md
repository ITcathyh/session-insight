...
独立运行、可部署到自己的电脑或开发机的 **Codex**、**Claude Code**、**TraeX** 会话分析器。
...
它跑在你自己的机器上：一个绑定回环地址的 Go 二进制，加一份 JSON 索引。不需要账号或数据库。远端部署通过 SSH 隧道访问，可从日志所在电脑同步会话。
...
打开 <http://127.0.0.1:4788>。选择 JSONL 文件并点击「同步所选」即可进入分析，或按来源和时间范围「扫描服务端」（默认最近 7 天）。目录上传会自动分批处理。首次准备环境可能下载依赖或 Go 工具链，session 分析在运行服务的机器上进行。

## 远端部署与同步

在远端仓库运行 `pnpm install --frozen-lockfile` 和 `pnpm insight`，然后在本机建立隧道：
...
打开 <http://127.0.0.1:4789>，点击「同步会话」，选择本机文件或目录、勾选需要同步的文件，再点击「同步所选」。用户点击后才上传，完成后更新分析。也可手动运行一次 `pnpm insight:sync --url http://127.0.0.1:4789`；命令行只需 Node.js，默认覆盖最近 7 天修改过的文件，`--days 0` 覆盖全部。完整 JSONL 经隧道传输，远端解析后删除原文。单文件限制 32 MiB，失败和跳过会明确报告；大型文件需要在日志所在机器扫描分析。页面「扫描服务端」读取远端目录，文件选择器从当前电脑选择，点击同步所选后才上传。详见[远端部署与同步说明](docs/session-insight.md#远端开发机部署与同步)。
...
- **只绑回环地址。** server 在非回环地址上会拒绝启动。它没有鉴权层，远端访问使用 SSH 隧道。
