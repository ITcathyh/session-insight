...
打开 <http://127.0.0.1:4788>。选择一个 JSONL 文件即可进入分析，或按来源和时间范围「扫描服务端」（默认最近 7 天）。目录上传会自动分批处理。首次准备环境可能下载依赖或 Go 工具链，session 分析在运行服务的机器上进行。

## 远端部署与同步

在远端仓库运行 `pnpm install --frozen-lockfile` 和 `pnpm insight`，然后在本机建立隧道并同步：

```bash
# 终端 1：保持运行；替换 SSH 登录目标
ssh -N -o ExitOnForwardFailure=yes -L 127.0.0.1:4789:127.0.0.1:4788 user@dev-host

# 终端 2：在本机仓库执行，每 60 秒同步变化文件
pnpm insight:sync --url http://127.0.0.1:4789 --interval 60
```

打开 <http://127.0.0.1:4789>。同步端只需 Node.js，默认覆盖最近 7 天修改过的文件；`--days 0` 覆盖全部。完整 JSONL 经隧道传输，远端解析后删除原文。单文件限制 32 MiB，失败和跳过会明确报告；大型文件需要在日志所在机器扫描分析。页面「扫描服务端」读取远端目录，文件选择器从当前电脑上传。详见[远端部署与同步说明](docs/session-insight.md#远端开发机部署与同步)。
